...
BJetModule::BJetModule(const string &name) : SubsysReco(name)
{
...
int BJetModule::Init(PHCompositeNode *topNode)
{
  ...
  _tree->Branch("particle_dca", _b_particle_dca,"particle_dca[particle_n]/F");
  ...
  _tree->Branch("track_dca2d_phi", _b_track_dca2d_phi, "track_dca2d_phi[track_n]/F");
  ...
  _tree->Branch("track_best_dca", _b_track_best_dca, "track_best_dca[track_n]/F");
...
int BJetModule::process_event(PHCompositeNode *topNode)
{
  ...
  */
  ...
  for (JetMap::Iter iter = truth_jets->begin(); iter != truth_jets->end(); ++iter) {
    ...
    if (this_jet->get_pt() < 10 || fabs(this_eta) > 2) continue;
    ...
    if (_verbose) 
      std::cout << " truth jet #" << ijet_t << ", pt / eta / phi = " << this_pt << " / " << this_eta << " / " << this_phi << std::endl;
  ...
  for ( PHG4TruthInfoContainer::ConstIterator iter = range.first; iter != range.second; ++iter ) {
    ...
    if (truth_pid == 12 || truth_pid == -12 || truth_pid == 16 || truth_pid == -16 || truth_pid == 14 || truth_pid == -14) continue;
	...
	if ((*p)->pdg_id() != truth_pid) continue;
	
	if ( fabs( truth_pt -  (*p)->momentum().perp() ) > 0.001) continue;
	if ( fabs( truth_eta -  (*p)->momentum().pseudoRapidity() ) > 0.001) continue;
	if ( fabs( truth_phi -  (*p)->momentum().phi() ) > 0.001) continue;

	HepMC::GenVertex *production_vertex = (*p)->production_vertex();
	_b_particle_dca[ _b_particle_n ] = production_vertex->point3d().perp();
      }
  ...
  for (SvtxTrackMap::Iter iter = trackmap->begin(); iter != trackmap->end(); ++iter) {
    ...
    if (track_pt < 0.5) continue;
    if (fabs(track_eta) > 1) continue;
    ...
    unsigned int truth_nclusters = trackeval->get_nclusters_contribution( track, g4particle );
    unsigned int truth_embed_id = truthinfo->isEmbeded( g4particle->get_track_id() );
